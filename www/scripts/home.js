...
$(document).ready(() => {
  ...
  $(".newServiceContainerOuterBodyFooter .btnClose").on(
    "click",
    function (event) {
      event.stopPropagation();
      event.preventDefault();
  ...
  $(".newServiceContainerOuterBodyFooter .btnSave").on(
    "click",
    function (event) {
      event.stopPropagation();
      event.preventDefault();
  ...
  $(".newCreateServiceContainerOuterBodyFooter .btnClose").on(
    "click",
    function (event) {
      event.stopPropagation();
      event.preventDefault();
  ...
  $(".newCreateServiceContainerOuterBodyFooter .btnSave").on(
    "click",
    function (event) {
      event.stopPropagation();
      event.preventDefault();
  ...
  $("#inputEquipmentType").on("change", function (event) {
    ...
    if (option === "100") {
      dropEquipmentTypeTextareaContent.removeClass(
        "textareaEquipmentTypeDescription"
      );
      return;
    }

    dropEquipmentTypeTextareaContent.addClass(
      "textareaEquipmentTypeDescription"
    );
  });
  ...
  $("#inputEquipmentType1").on("change", function (event) {
    ...
    if (option === "100") {
      dropEquipmentTypeTextareaContent.removeClass(
        "textareaEquipmentTypeDescription1"
      );
      return;
    }
  ...
  $("#inputEquipmentProcedure").on("change", function (event) {
    ...
    let dropEquipmentProcedureTextareaContent =
      dropEquipmentProcedureTextarea.parent();

    dropEquipmentProcedureTextarea.val("");

    if (option === "100") {
      dropEquipmentProcedureTextareaContent.removeClass(
        "textareaEquipmentProcedureDescription"
      );
      return;
    }

    dropEquipmentProcedureTextareaContent.addClass(
      "textareaEquipmentProcedureDescription"
    );
  });
  ...
  $("#inputEquipmentProcedure1").on("change", function (event) {
    ...
    if (option === "100") {
      dropEquipmentProcedureTextareaContent.removeClass(
        "textareaEquipmentProcedureDescription1"
      );
      return;
    }
  ...
  $(".newServiceContainerOuterBodyFooter .btnReopen").on(
    "click",
    function (event) {
      event.stopPropagation();
      event.preventDefault();
  ...
  $(document).on(
    "dragstart",
    ".jobEditCardDetailsPriority .card-body-body .badge",
    function (event) {
  ...
  $(document)
    .on(
      "dragover",
      ".jobEditCardDetailsPriority .card-body-header",
      function (event) {
        event.stopPropagation();
        event.preventDefault();
      }
  ...
  $(document).on(
    "dragstart",
    ".jobCreateCardDetailsPriority .card-body-body .badge",
    function (event) {
      event.originalEvent.dataTransfer.effectAllowed = "move";
      event.originalEvent.dataTransfer.setData(
        "code",
  ...
  $(document)
    .on(
      "dragover",
      ".jobCreateCardDetailsPriority .card-body-header",
      function (event) {
        event.stopPropagation();
        event.preventDefault();
      }
  ...
  $(document).on("dragstart", "#jobsTable tbody tr", function (event) {
    ...
    event.originalEvent.dataTransfer.setData(
      "rowInfo",
      $(event.target).attr("data-rowinfo")
    );
  });

  $(document)
    .on("dragover", "#jobsTable tbody tr", function (event) {
      event.stopPropagation();
      event.preventDefault();
    })
    ...
    .on("drop", "#jobsTable tbody tr", function (event) {
      const startRowInfo = JSON.parse(
        event.originalEvent.dataTransfer.getData("rowInfo")
      );
      let endRowInfo = $(event.target);
      if (endRowInfo[0].tagName === "TD") {
        endRowInfo = endRowInfo.parent();
      } else if (
        endRowInfo[0].tagName === "SPAN" ||
        endRowInfo[0].tagName === "I" ||
        endRowInfo[0].tagName === "SMALL"
      ) {
...
function buildInitialPageState() {
  ...
  initPageState[1].forEach((jobEquipment) => {
    dropEquipmentType.append(
      `<option value='${jobEquipment.code}'>${jobEquipment.description}</option>`
    );
    dropEquipmentType2.append(
      `<option value='${jobEquipment.code}'>${jobEquipment.description}</option>`
    );
  });
  ...
  initPageState[3].forEach((brand) => {
    dropEquipmentBrand.append(
      `<option value='${brand.code}'>${brand.description}</option>`
    );
    dropEquipmentBrand2.append(
      `<option value='${brand.code}'>${brand.description}</option>`
    );
  });
  ...
  initPageState[2].forEach((jobEquipment) => {
    dropEquipmentProcedure.append(
      `<option value='${jobEquipment.code}'>${jobEquipment.description}</option>`
    );
    dropEquipmentProcedure2.append(
      `<option value='${jobEquipment.code}'>${jobEquipment.description}</option>`
    );
  });
  ...
  tableJobs = table.DataTable({
    ...
    fnRowCallback: function (row, rowData) {
      ...
      $(row)
        .attr("scope", "row")
        .attr("draggable", "true")
        .attr("data-rowinfo", JSON.stringify(rowInfo));
    },
    columnDefs: [
    ...
    columns: [
      ...
      {
        data: null,
        render: function (data, type, _, settings) {
          if (type === "display") {
...
function openNewServiceModal() {
  ...
  textareaEquipmentProcedure1
    .parent()
    .addClass("textareaEquipmentProcedureDescription1");
...
function saveJobInformation() {
  ...
  job.priority = $(".jobEditCardDetailsPriority")
    .find(".card-body-header .badge")
    .attr("data-code");
...
function createJobInformation() {
  ...
  job.priority = $(".jobCreateCardDetailsPriority")
    .find(".card-body-header .badge")
    .attr("data-code");
...
function editJobOpenModal(jobId) {
  let job = jobsArray.filter((j) => {
    return j.JOB_ID === jobId;
  })[0];

  let dropEquipmentType = $("#inputEquipmentType");
  dropEquipmentType
    .find(`option[value='${job.EQUIPMENT_TYPE}']`)
    .prop("selected", true);

  let dropEquipmentBrand = $("#inputEquipmentBrand");
  dropEquipmentBrand
    .find(`option[value='${job.EQUIPMENT_BRAND}']`)
    .prop("selected", true);
  ...
  let dropEquipmentProcedure = $("#inputEquipmentProcedure");
  dropEquipmentProcedure
    .find(`option[value='${job.EQUIPMENT_PROCEDURE}']`)
    .prop("selected", true);
  ...
  let editJobStatus = $("#inputJobStatus");
  editJobStatus
    .find(`option[value='${job.STATUS_PROGRESS_CODE}']`)
    .prop("selected", true);
  ...
  let jobEditCardDetailsPriority = $(".jobEditCardDetailsPriority").find(
    ".card-body-body"
  );
  ...
  if (job.STATUS_PROGRESS_CODE === "4") {
    ...
    divInfoFinalised
      .find(".alert")
      .html(
        `Concluido por: ${job.USER_FINALISED} <small>(${job.DATE_FINISHED})</small>`
      );
...
function setPriorityCard(prioritySelected, classCard) {
  ...
  let jobCardDetailsPriorityHeader =
    jobCardDetailsPriority.find(".card-body-header");
  let jobCardDetailsPriorityBody =
    jobCardDetailsPriority.find(".card-body-body");
  jobCardDetailsPriorityHeader.empty();
  jobCardDetailsPriorityBody.empty();

  priorityList
    .filter((p) => {
      return p.code === prioritySelected;
    })
    .forEach((p) => {
      if (p.code === "1") {
        jobCardDetailsPriorityHeader.append(
          `<span data-code="${p.code}" class="badge badge-danger">${p.description}</span>`
        );
      } else if (p.code === "2") {
        jobCardDetailsPriorityHeader.append(
          `<span data-code="${p.code}" class="badge badge-warning">${p.description}</span>`
        );
      } else if (p.code === "3") {
        jobCardDetailsPriorityHeader.append(
          `<span data-code="${p.code}" class="badge badge-light">${p.description}</span>`
        );
      } else if (p.code === "4") {
        jobCardDetailsPriorityHeader.append(
          `<span data-code="${p.code}" class="badge badge-secondary">${p.description}</span>`
        );
      }
    });
  ...
  priorityList
    ...
    .forEach((p) => {
      if (p.code === "1") {
        jobCardDetailsPriorityBody.append(
          `<span draggable="true" data-code="${p.code}" class="badge badge-danger">${p.description}</span>`
        );
      } else if (p.code === "2") {
        jobCardDetailsPriorityBody.append(
          `<span draggable="true" data-code="${p.code}" class="badge badge-warning">${p.description}</span>`
        );
      } else if (p.code === "3") {
        jobCardDetailsPriorityBody.append(
          `<span draggable="true" data-code="${p.code}" class="badge badge-light">${p.description}</span>`
        );
      } else if (p.code === "4") {
        jobCardDetailsPriorityBody.append(
          `<span draggable="true" data-code="${p.code}" class="badge badge-secondary">${p.description}</span>`
        );
      }
    });
}
...
function editOrderPriority(startRowInfo, endRowInfo, callback) {
  ...
  xhr.send(
    JSON.stringify({ startRowInfo: startRowInfo, endRowInfo: endRowInfo })
  );
}
