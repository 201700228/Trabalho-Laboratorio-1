...
function createNewUserModalVerification() {
  ...
  if (
    usersArray.filter((u) => {
      return (
        u.userName.replace(" ", "").toUpperCase() ===
  ...
  if (
    usersArray.filter((u) => {
      return (
        u.email.replace(" ", "").toUpperCase() ===
        email.replace(" ", "").toUpperCase()
      );
    }).length
  ) {
    showHideModalErrorMessage(
  ...
  const user = new User(null, userName, name, email, role, confirmPassword);
...
function openUserModalToEdit(userId) {
  const user = usersArray.filter((u) => {
    return u.id === userId;
  })[0];
  ...
  modal
    .find(`#inpEditUserType option[value='${user.roleCode}']`)
    .prop("selected", true);
...
function checkUserModalToDelete() {
  ...
  modalEdit.on("hidden.bs.modal", function (e) {
    ...
    modalMessage
      .find(".modal-body p")
      .text("Tem a certeza que quer remover este utilizador?");
...
function checkUserModalToSave() {
  ...
  if (
    usersArray.filter((u) => {
      return (
        u.email.replace(" ", "").toUpperCase() ===
          email.replace(" ", "").toUpperCase() && u.id !== userId
      );
    }).length
  ) {
    showHideModalErrorMessage(
  ...
  const user = new User(userId, userName, name, email, role, null);
