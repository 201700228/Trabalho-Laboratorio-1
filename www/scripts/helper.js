...
function initUserDetailsAndRoles() {
  ...
  $("#userLabelIcon").text(
    `${userDetails.name} (${userDetails.roleDescription})`
  );
...
function initWebSocketMessaging() {
  ...
  $("#buttonSendMessage").on("click", function (event) {
    ...
    const menssageToSend = new MessageSocket(
      "send",
      0,
      userDetails.id,
      messagingMode.UserSelected.ID,
      message
    );
...
function initWebSocketMessagingDataPage() {
  ...
  messagingMode.Users.forEach((user) => {
    ul.append(
      `<a class='dropdown-item' href='#' onclick='Javascript:switchUserMessaging(${JSON.stringify(
        user
      )});'>${user.NAME}</a>`
    );
  });
...
function updateUserMessagingContainer() {
  ...
  $("#containerDirectMenssageTitle").html(
    `Mensagens <span>(${UserSelected.NAME})</span>`
  );
  ...
  let messages = messagingMode.Messages.filter((m) => {
    return (
      m.MESSAGE_TO_ID === UserSelected.ID ||
      m.MESSAGE_FROM_ID === UserSelected.ID
    );
  });
...
function userMessageHtmlCode(from, date, message, us) {
  return `
        <div class="message-container-info${
          us ? "" : " message-container-right"
        }">
            <label class="message-title">
                <span>${
                  us ? "<small>(eu)</small> " + from : from
