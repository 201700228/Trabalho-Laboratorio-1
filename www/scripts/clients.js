...
$(document).ready(() => {
  ...
  $(document).on(
    "click",
    "#clientsTable tbody tr td:last-child i",
    function (e) {
      e.preventDefault();
      e.stopPropagation();
...
function openClientModalToEdit(clientId) {
  const client = clientsArray.filter((c) => {
...
function checkClientModalToSave() {
  ...
  if (
    clientsArray.filter((c) => {
      return (
        c.name.replace(" ", "").toUpperCase() ===
          client.name.replace(" ", "").toUpperCase() && c.id !== clientId
      );
    }).length
  ) {
  ...
  if (
    clientsArray.filter((c) => {
      return (
        c.email.replace(" ", "").toUpperCase() ===
          client.email.replace(" ", "").toUpperCase() && c.id !== clientId
      );
    }).length
  ) {
    showHideModalErrorMessage(
      modal,
      true,
      "Um cliente com este email já existe"
    );
    return;
  }
  ...
  editClientAjax(client, (status) => {
    if (!status) {
      showHideModalErrorMessage(
        modal,
        true,
        "Algo correu mal. Tente outravez."
      );
      return;
    }
    loadAllClients();
...
function checkClientModalToDelete() {
  ...
  modalEdit.on("hidden.bs.modal", function (e) {
    ...
    modalMessage
      .find(".modal-body p")
      .text("Tem a certeza que quer remover este cliente?");
...
function createNewClientModalVerification() {
  ...
  if (
    clientsArray.filter((c) => {
      return (
        c.name.replace(" ", "").toUpperCase() ===
        client.name.replace(" ", "").toUpperCase()
      );
    }).length
  ) {
    showHideModalErrorMessage(modal, true, "Este cliente já existe");
  ...
  if (
    clientsArray.filter((c) => {
      return (
        c.email.replace(" ", "").toUpperCase() ===
        client.email.replace(" ", "").toUpperCase()
      );
    }).length
  ) {
    showHideModalErrorMessage(
  ...
  getClientsAjax((clients) => {
    ...
    clientTable = table.DataTable({
      ...
      columns: [
        ...
        {
          data: null,
          render: function (data, type, _, __) {
            if (type === "display") {
              ...
              let status =
                data.TOTAL_JOBS_FINALISED === 1
                  ? "1 finalisado em "
                  : data.TOTAL_JOBS_FINALISED + " concluído em ";
